...
export default function CreateLab() {
  ...
  const [formData, setFormData] = useState({
    name: '',
    testsOffered: [],
    homeSampleCollection: null, // 'Yes' or 'No'
    openHours: '',
    contactNo: '',
    location: '',
    images: [],
  ...
  const handleAddTest = () => {
    if (testName.trim()) {
      setFormData(prev => ({ ...prev, testsOffered: [...prev.testsOffered, testName.trim()] }));
      setTestName('');
    }
  };

  const handleRemoveTest = (index) => {
    setFormData(prev => ({ ...prev, testsOffered: prev.testsOffered.filter((_, i) => i !== index) }));
  };

  const pickImages = async () => {
    try {
      const result = await ImagePicker.launchImageLibraryAsync({
        // ✅ FIX: Use ImagePicker.MediaTypeOptions.Images instead of MediaType.Images
  ...
  const validateForm = () => {
    ...
    if (formData.homeSampleCollection === null) {
      setError('Please specify if Home Sample Collection is available.');
      return false;
    }
    setError('');
    return true;
  ...
  const handleSubmit = async () => {
    ...
    const submissionData = {
        ...formData,
        homeSampleCollection: formData.homeSampleCollection === 'Yes'
    };
    
    try {
      const API_URL = 'https://locato-backend-wxjj.onrender.com/api/labs';
      const response = await axios.post(API_URL, submissionData);
      
      if (response.status === 201) {
        Alert.alert('Success!', 'Lab listing has been created.');
        router.back();
      }
    } catch (err) {
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView contentContainerClassName="p-6">
          <View className="flex-row items-center mb-6">
            ...
            <Text className="text-3xl font-bold text-gray-800 ml-2">Add New Lab</Text>
          </View>
          ...
          <View className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 mb-6">
            <Text className="text-xl font-semibold text-gray-700 mb-4">Lab Information</Text>
            <FormField label="Name" placeholder="e.g., Asiri Laboratories" value={formData.name} onChangeText={v => handleChange('name', v)} />
            ...
            <FormField label="Open Hours" placeholder="e.g., 8:00 AM - 6:00 PM" value={formData.openHours} onChangeText={v => handleChange('openHours', v)} />
          </View>
          
          <View className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 mb-6">
             <Text className="text-xl font-semibold text-gray-700 mb-4">Services Offered</Text>
             <SelectionField label="Home Sample Collection" options={['Yes', 'No']} selectedValue={formData.homeSampleCollection} onSelect={v => handleChange('homeSampleCollection', v)} />
             
             <View className="mt-4">
               <Text className="text-gray-600 mb-2 font-medium">Tests Offered</Text>
               <View className="flex-row gap-2 items-center mb-2">
                   <TextInput className="flex-1 bg-gray-100 rounded-xl p-4 text-black border border-gray-200" placeholder="e.g., Full Blood Count" value={testName} onChangeText={setTestName} />
                   <TouchableOpacity onPress={handleAddTest} className="bg-blue-500 p-3 rounded-full h-14 w-14 justify-center items-center"><Ionicons name="add" size={28} color="white" /></TouchableOpacity>
               </View>
               <View className="flex-row flex-wrap gap-2">
                   {formData.testsOffered.map((item, index) => (
                       <View key={index} className="flex-row items-center bg-blue-50 px-3 py-1 rounded-full">
                           <Text className="text-blue-800">{item}</Text>
                           <TouchableOpacity onPress={() => handleRemoveTest(index)} className="ml-2"><Ionicons name="close-circle" size={18} color="red" /></TouchableOpacity>
                       </View>
                   ))}
               </View>
             </View>
          ...
          <View className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 mb-6">
            ...
            <TouchableOpacity onPress={pickImages} className="border-2 border-dashed border-gray-300 rounded-xl p-6 items-center justify-center bg-gray-50 mb-4">
                <Ionicons name="cloud-upload-outline" size={40} color="gray" /><Text className="text-gray-500 mt-2">Tap to upload lab images</Text>
            </TouchableOpacity>
          ...
          <TouchableOpacity onPress={handleSubmit} disabled={loading} className="bg-blue-600 w-full rounded-xl py-4 items-center mt-4">
            {loading ? <ActivityIndicator color="#FFFFFF" /> : <Text className="text-white font-bold text-lg">Create Lab Listing</Text>}
          </TouchableOpacity>
        </ScrollView>
