...
// Main Component
export default function CreateElectronics() {
  ...
  const [tempItem, setTempItem] = useState({ product: '', service: '', brand: '' });
...
encoding: 'base64',
  ...
  const handleSubmit = async () => {
    ...
    const submissionData = {
      ...formData,
      repairServices: formData.repairServices === 'Yes',
      onlineShopping: formData.onlineShopping === 'Yes',
    };
    try {
      const API_URL = 'https://locato-backend-wxjj.onrender.com/api/electronics';
      await axios.post(API_URL, submissionData);
      Alert.alert('Success!', 'Electronics store listing has been created.');
      router.back();
    } catch (err) {
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView contentContainerClassName="p-6">
          <View className="flex-row items-center mb-6">
            ...
            <Text className="text-3xl font-bold text-gray-800 ml-2">Add New Electronics Store</Text>
          </View>
          ...
          <View className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 mb-6">
            ...
            <FormField label="Name" placeholder="e.g., TechTrend Innovations" value={formData.name} onChangeText={v => handleChange('name', v)} />
            <FormField label="Location" placeholder="e.g., Colombo 5" value={formData.location} onChangeText={v => handleChange('location', v)} />
            <FormField label="Description" multiline={true} placeholder="e.g., Your one-stop shop for electronics" value={formData.description} onChangeText={v => handleChange('description', v)} />
          </View>

          <View className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 mb-6">
            <Text className="text-xl font-semibold text-gray-700 mb-4">Store Details</Text>
            <DynamicListField label="Products Sold" placeholder="e.g., Smartphones" items={formData.productsSold} item={tempItem.product} setItem={v => setTempItem(p => ({ ...p, product: v }))} onAddItem={() => { if (handleAddItem('productsSold', tempItem.product)) setTempItem(p => ({ ...p, product: '' })); }} onRemoveItem={(i) => handleRemoveItem('productsSold', i)} />
            <DynamicListField label="Services Offered" placeholder="e.g., Repairs" items={formData.servicesOffered} item={tempItem.service} setItem={v => setTempItem(p => ({ ...p, service: v }))} onAddItem={() => { if (handleAddItem('servicesOffered', tempItem.service)) setTempItem(p => ({ ...p, service: '' })); }} onRemoveItem={(i) => handleRemoveItem('servicesOffered', i)} />
            <DynamicListField label="Brand Purchases" placeholder="e.g., Samsung" items={formData.brandPurchases} item={tempItem.brand} setItem={v => setTempItem(p => ({ ...p, brand: v }))} onAddItem={() => { if (handleAddItem('brandPurchases', tempItem.brand)) setTempItem(p => ({ ...p, brand: '' })); }} onRemoveItem={(i) => handleRemoveItem('brandPurchases', i)} />
            <FormField label="Warranty Info" multiline={true} placeholder="e.g., 1-year warranty on smartphones" value={formData.warrantyInfo} onChangeText={v => handleChange('warrantyInfo', v)} />
          </View>

          <View className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 mb-6">
            <Text className="text-xl font-semibold text-gray-700 mb-4">Operations & Contact</Text>
            <FormField label="Opening Hours" placeholder="e.g., Mon-Sat 9AM-7PM" value={formData.openingHours} onChangeText={v => handleChange('openingHours', v)} />
            <FormField label="Delivery Options" multiline={true} placeholder="e.g., Free delivery within 10km" value={formData.deliveryOptions} onChangeText={v => handleChange('deliveryOptions', v)} />
            ...
            <FormField label="Contact Email" placeholder="e.g., contact@techtrend.com" value={formData.contactInfo.email} onChangeText={v => handleChange('contactInfo.email', v)} keyboardType="email-address" />
            <FormField label="Social Media" placeholder="e.g., instagram.com/techtrend" value={formData.contactInfo.socialMedia} onChangeText={v => handleChange('contactInfo.socialMedia', v)} />
            <SelectionField label="Repair Services" options={['Yes', 'No']} selectedValue={formData.repairServices} onSelect={v => handleChange('repairServices', v)} />
            ...
            <FormField label="Customer Support Availability" placeholder="e.g., 24/7 phone support" value={formData.customerSupportAvailability} onChangeText={v => handleChange('customerSupportAvailability', v)} />
          </View>
          ...
          <TouchableOpacity onPress={handleSubmit} disabled={loading} className="bg-blue-600 w-full rounded-xl py-4 items-center mt-4">
            {loading ? <ActivityIndicator color="#FFFFFF" /> : <Text className="text-white font-bold text-lg">Create Electronics Store Listing</Text>}
          </TouchableOpacity>
        </ScrollView>
