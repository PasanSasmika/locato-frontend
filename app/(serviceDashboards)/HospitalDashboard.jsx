...
export default function CreateHospital() {
  ...
  const validateForm = () => {
    const required = ['name', 'hospitalType', 'contactNo', 'location', 'coordinates'];
    for (const field of required) {
      if (field === 'coordinates' && (!formData.coordinates.latitude || !formData.coordinates.longitude)) {
  ...
  const handleSubmit = async () => {
    ...
    const submissionData = {
      ...formData,
      emergency247: formData.emergency247 === 'Yes'
    };
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView contentContainerClassName="p-6">
          ...
          <View className="bg-white p-5 rounded-2xl shadow-lg border border-gray-100 mb-6">
            ...
            <Text className="text-gray-600 mb-4 font-medium">Tap on the map to set the precise location of the hospital. 🗺️</Text>
              ...
              >
                {formData.coordinates.latitude && (
                  <Marker
                    ...
                    title={formData.name || 'New Hospital Location'}
                    pinColor="#2563EB"
                  />
                )}
              </MapView>
          ...
          <TouchableOpacity onPress={handleSubmit} disabled={loading} className="bg-blue-600 w-full rounded-xl py-4 items-center mt-4">
            {loading ? <ActivityIndicator color="#FFFFFF" /> : <Text className="text-white font-bold text-lg">Create Hospital Listing</Text>}
          </TouchableOpacity>
        </ScrollView>
